<?php
$sql_consult = mysql_query("SELECT * FROM tb_cotacao WHERE id='".$_GET['id']."'") or die (mysql_error());
...
$sql_cliente = mysql_query("SELECT * FROM ta_cliente_fornecedor WHERE id='".$row_consult['id_cliente']."'") or die (mysql_error());
$row_cliente = mysql_fetch_assoc($sql_cliente);

$sql_loc = mysql_query("SELECT * FROM ta_cliente_local WHERE id='".$row_consult['id_clienteLocal']."'") or die (mysql_error());
$row_loc = mysql_fetch_assoc($sql_loc);

$sql_situ = mysql_query("SELECT * FROM tb_situacao_cotacao WHERE id='".$row_consult['id_situacao']."'") or die (mysql_error());
$row_situ = mysql_fetch_assoc($sql_situ);

$sql_operador = mysql_query("SELECT * FROM tb_usuario WHERE id='".$row_consult['id_operador']."'") or die (mysql_error());
$row_operador = mysql_fetch_assoc($sql_operador);

$sql_gr = mysql_query("SELECT * FROM ta_material_grupo WHERE id='".$row_consult['id_materialGrupo']."'") or die (mysql_error());
$row_gr = mysql_fetch_assoc($sql_gr);
$grupo = $row_gr['ds_materialGrupo'];

$sql_priori = mysql_query("SELECT * FROM tb_prioridade_compra WHERE id='".$row_consult['id_prioridade']."'") or die (mysql_error());
...
?>
	   ...
	   <form id="form_add_material_compra" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="tipo" value="<?php echo $row_consult['tipo'] ?>" />
             ...
             <div class="col-md-4">
				...
				<?php
				$sql_marca = mysql_query("SELECT * FROM ta_material_marca WHERE id_grupo='".$row_consult['id_materialGrupo']."' ORDER BY marca") or die (mysql_error());
				while ($row_marca = mysql_fetch_array($sql_marca)) {
				?>		
  ...
  $sql_materiais = mysql_query("SELECT * FROM tb_materiais_cotacao WHERE id_cotacao='".$_GET['id']."' ORDER BY id ASC") or die (mysql_error());
  while ($dados = mysql_fetch_array($sql_materiais)) {
  
  $sql_consult2 = mysql_query("SELECT * FROM ta_material WHERE id='".$dados['id_material']."'") or die (mysql_error());
  $row_consult2 = mysql_fetch_assoc($sql_consult2);
  
  
	$sql_cl = mysql_query("SELECT * FROM ta_material_classe WHERE id='".$row_consult2['id_materialClasse']."'") or die (mysql_error());
	$row_cl = mysql_fetch_assoc($sql_cl);
	$classe = $row_cl['classe'];
	
	
	$sql_ma = mysql_query("SELECT * FROM ta_material_marca WHERE id='".$dados['id_marca']."'") or die (mysql_error());
	$row_ma = mysql_fetch_assoc($sql_ma);
	if($row_ma['marca'] == '') { $marca = "Sem marca"; } else { $marca = $row_ma['marca']; }

	$sql_unidade = mysql_query("SELECT * FROM ta_unidade WHERE id='".$row_consult2['id_unidade']."'") or die (mysql_error());
	$row_unidade = mysql_fetch_array($sql_unidade);  
